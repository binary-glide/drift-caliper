"""Step definitions for BIN-65: fit EWMA control limits from the baseline.

Binds to
``tests/bdd/features/baseline/ewma-control-limit-fitting.feature`` via
``tests/bdd/test_ewma_control_limit_fitting.py``. Error assertions follow
ADR-002/ADR-008: type + required ``context`` keys only -- never message
text. See ``tests/unit/baseline/test_ewma_fitting.py``'s module docstring
...
mechanism) -- they apply identically here; not re-argued in this file to
avoid duplication.
...
sets, the ordering property, the published-ARL numeric proof) live in
``tests/unit/baseline/test_ewma_fitting.py`` and
``tests/unit/baseline/test_ewma_arl_published_values.py`` instead, per
...
_VALID_SMOOTHING_PARAM = 0.3
_CUSTOM_SMOOTHING_PARAM = 0.6  # distinct from DEFAULT_SMOOTHING_PARAM, on purpose

_IDENTICAL_SCORE = 0.62
_ZERO_VARIANCE_REASON = "zero_variance"

_INVALID_SMOOTHING_PARAM_VALUES = {
    "zero": 0.0,
    "negative": -0.1,
    "above the valid upper bound": MAX_SMOOTHING_PARAM * 2,
}
...
_SMOOTHING_PARAM_BOUNDARIES = {
    "the smallest valid value": MIN_SMOOTHING_PARAM,
...
class FittingOutcome:
    """A successful ``fit_ewma()`` call, with the baseline it was fitted from."""
...
class FittingAttempt:
    """The outcome of a ``fit_ewma()`` call expected to fail."""
...
@given(
    "the engineer has fitted EWMA control limits from a Phase I baseline",
...
def a_baseline_already_fitted() -> FittingOutcome:
    ...
    result = fit_ewma(
        baseline, target_arl=_VALID_TARGET_ARL, smoothing_param=_VALID_SMOOTHING_PARAM
    )
...
@when(
    "they fit EWMA control limits with a specified false alarm tolerance",
    target_fixture="outcome",
)
...
# --- When: fit with a custom smoothing parameter ------------------------------------


@when(
    "they fit EWMA control limits with a specified false alarm tolerance and a "
    "custom smoothing parameter",
    target_fixture="outcome",
)
def fit_with_custom_smoothing_parameter(baseline: Baseline) -> FittingOutcome:
    result = fit_ewma(
        baseline, target_arl=_VALID_TARGET_ARL, smoothing_param=_CUSTOM_SMOOTHING_PARAM
    )
    return FittingOutcome(baseline=baseline, result=result)


# --- When: fit without specifying a smoothing parameter -----------------------------


@when(
    "they fit EWMA control limits with a specified false alarm tolerance without "
    "specifying a smoothing parameter",
    target_fixture="outcome",
)
def fit_without_smoothing_parameter(baseline: Baseline) -> FittingOutcome:
...
@when("they attempt to fit EWMA control limits", target_fixture="attempt")
def attempt_to_fit(baseline: Baseline) -> FittingAttempt:
    return _capture_fitting_error(baseline, target_arl=_VALID_TARGET_ARL)


# --- When: attempt to fit with an invalid smoothing parameter ----------------------


@when(
    parsers.parse(
        "they attempt to fit EWMA control limits with a smoothing parameter that "
        "is {invalid_case}"
    ),
    target_fixture="attempt",
)
def attempt_to_fit_with_invalid_smoothing_parameter(
    baseline: Baseline, invalid_case: str
) -> FittingAttempt:
    invalid_value = _INVALID_SMOOTHING_PARAM_VALUES[invalid_case]
    return _capture_fitting_error(
        baseline, target_arl=_VALID_TARGET_ARL, smoothing_param=invalid_value
    )
...
@when(
    parsers.parse(
        "they attempt to fit EWMA control limits with a false alarm tolerance "
...
@when(
    "they attempt to fit EWMA control limits without specifying a false alarm "
    "tolerance",
    target_fixture="attempt",
)
...
# --- When: fit with a smoothing parameter at a named boundary ----------------------


@when(
    parsers.parse(
        "they fit EWMA control limits with a specified false alarm tolerance and "
        "a smoothing parameter at {boundary}"
    ),
    target_fixture="outcome",
)
def fit_with_smoothing_parameter_at_boundary(
    baseline: Baseline, boundary: str
) -> FittingOutcome:
    boundary_value = _SMOOTHING_PARAM_BOUNDARIES[boundary]
    result = fit_ewma(
        baseline, target_arl=_VALID_TARGET_ARL, smoothing_param=boundary_value
    )
    return FittingOutcome(baseline=baseline, result=result)
...
@when(
    parsers.parse(
        "they fit EWMA control limits with a false alarm tolerance at {boundary}"
    ),
    target_fixture="outcome",
)
def fit_with_target_arl_at_boundary(
    ...
    result = fit_ewma(baseline, target_arl=boundary_value)
    return FittingOutcome(baseline=baseline, result=result)
...
@then("the result reports the smoothing parameter that was used")
def result_reports_the_smoothing_parameter(outcome: FittingOutcome) -> None:
    assert isinstance(outcome.result.smoothing_param, float)
...
# --- Then: custom vs. default smoothing parameter -----------------------------------


@then(
    "the result uses the engineer's specified smoothing parameter rather than the "
    "library default"
)
def result_uses_the_custom_smoothing_parameter(outcome: FittingOutcome) -> None:
    assert outcome.result.smoothing_param == _CUSTOM_SMOOTHING_PARAM
    assert outcome.result.smoothing_param != DEFAULT_SMOOTHING_PARAM


@then("the result uses the library's default smoothing parameter")
def result_uses_the_default_smoothing_parameter(outcome: FittingOutcome) -> None:
    assert outcome.result.smoothing_param == DEFAULT_SMOOTHING_PARAM


@then("the result reports which smoothing parameter was used")
def result_reports_which_smoothing_parameter(outcome: FittingOutcome) -> None:
    assert isinstance(outcome.result.smoothing_param, float)
...
@then(
    "the error explains that EWMA control limits require score variation in the "
    "baseline"
)
...
# --- Then: invalid parameter (smoothing param / target_arl outlines, missing) -------
...
def error_identifies_invalid_smoothing_parameter(attempt: FittingAttempt) -> None:
    error = attempt.error
...
def no_control_limits_are_produced(attempt: FittingAttempt) -> None:
    # The raise itself is the proof -- no FittedEWMA exists in this branch.
...
def the_artefact_reports_its_original_values(mutation: MutationAttempt) -> None:
    ...
    assert result.smoothing_param == mutation.snapshot["smoothing_param"]
