...
object (``caliper.monitoring.domain.monitor._safe_repr`` falls back through
...
``Baseline.record()`` and ``compare_provenance()`` (``caliper.baseline``), and
``Monitor.record()`` (``caliper.monitoring``). ``caliper.monitoring`` already
imports directly from ``caliper.baseline.domain`` for several other
...
``src/caliper/baseline/`` imports from ``caliper.monitoring``). Placing this
...
``src/caliper/errors.py`` treatment -- that placement is justified there by
...
``src/caliper/`` top level unless a *third*, independent bounded context
...
def probe_fields(candidate: object, names: Sequence[str]) -> FieldsProbe:
    ...
    (byte-identical in ``caliper.baseline.domain.baseline`` and
    ``caliper.monitoring.domain.monitor`` before BIN-127) -- both call sites
