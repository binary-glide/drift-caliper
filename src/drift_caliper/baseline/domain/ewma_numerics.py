...
``fit_ewma`` (``drift_caliper.baseline.domain.ewma_fitting``) is a public entry
...
# is now drift_caliper.baseline.domain.parameter_guards.MIN_TARGET_ARL (100.0),
...
#
# Internal-only: absent from both drift_caliper.__all__ and
# drift_caliper.baseline.__all__, and not reachable via hasattr on either package
