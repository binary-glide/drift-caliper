...
``drift_caliper.baseline.domain.attribute_probe`` for that guard, applied at
...
def log_receiver(signal: MonitoringResult) -> None:
    """Log ``signal`` as a ``WARNING``-level entry on ``"drift_caliper.monitoring"``.
    ...
    Attach a handler to ``logging.getLogger("drift_caliper.monitoring")`` to
