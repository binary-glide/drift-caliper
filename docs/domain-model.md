...
10. **A `Monitor` over a `FittedBernoulliCUSUM` runs exactly the integer chain whose ARLs the artefact reports.** *(ADR-014 Amendment 2, Decision 14.4–14.5; corrigendum C3/C4/C6. **Built on unmerged PR #29 at `3bd013d`.** Before that, at `5b7e3a1`, PR #29 accumulated `max(0, S + x − r)` in binary floating point from the derived float fields. The ADR measured that path signalling earlier than the exact chain: 211 of 400 in-control runs at m=300 f=3, never later. That is defect 3.)*
...
*(BIN-133, following ADR-012/013/014, ADR-014 Amendment 1 (Decisions 7–11, 2026-09-23) ADR-014 Amendment 2 (Decisions 12–19, ratified 2026-09-24) and the **Amendment 2 corrigendum (C1–C15; C1–C14 ratified 2026-09-24, and C15.1/C15.2 settled corrections within them)**. **Not on `trunk`.** For what is built where, see "BIN-133 implementation status" at the end of this entry.)*
...
| `expected_detection_arl` | `float \| None` | ADR-013 §4's detection-performance disclosure, **amended by Decision 15 and by corrigendum C13**. It is the exact ARL₁ of the constructed chart, evaluated at **the shift the checked arm(s) are tuned to detect**. It is first-class and not routed through `FittingAdvisory` (Decision 4). ~~Unconditional~~: since C13 it is **`None` exactly when that shift lies at or inside the chart's design rate**, and then the figure would describe no detection at all. The rule: <br>• `"lower"`/`"two_sided"`: `None` when f ≥ 1 and `p̂·M ≤ p_U`. <br>• `"upper"`: `None` when `p̂/M ≥ p_L`. <br>This is decided from `(p̂, M, p_U, p_L)` **before any solve**. It is `None` even when the solve would have succeeded: m=100 f=25 M=1.2 would give 2,843,692. Each `None` is paired with a `FittingAdvisory` (see `advisories`). The boundary is included, because at equality the figure equals the in-control run length. At f = 0 the degradation figure is evaluated at `p_U·M > p_U`, so it is always reported. <br>• `"lower"`: the lower arm at failure rate `p̂ × M`, or `p_U × M` when f = 0. That includes a `"lower"` produced by a two-sided request at f = 0. <br>• `"two_sided"` (f ≥ 1 only): the ordinary joint chain at the single rate `p̂ × M`. <br>• `"upper"` (f ≥ 1 only): the upper arm at failure rate **`p̂ / M`**, the improvement it is tuned for. It is **not** `p̂ × M`: the shipped code evaluated the upper-only chart at a degradation it is built never to signal on. At m=150 f=40 that reported 357,451.6. The tuned figure, with the upper arm at `p_L`, is 72.1128 (corrigendum C1). Amendment 2's 32.8 was computed with the upper arm at `p_U` and is withdrawn. <br>Decision 15's `p_U / M` fallback for `"upper"` at f = 0 is **withdrawn** (Decision 19.2): `"upper"` refuses at f = 0 (row F15), so the fallback is unreachable. So is the `"two_sided"` row's f = 0 fallback (corrigendum C8), because a two-sided request at f = 0 becomes `"lower"` and takes the `"lower"` row |
| `expected_improvement_detection_arl` | `float \| None` | *(New, Decision 19.6; narrowed by corrigendum C13.)* **Non-`None` exactly when the chart checks both arms** (`direction == "two_sided"`, which implies f ≥ 1) **and `p̂/M < p_L`**. When `p̂/M ≥ p_L` the improvement rate lies inside `[p_L, p_U]`, so by 19.4's coupling the figure is no faster than the false-alarm floor `B`. It is then `None`, with `FittingAdvisory(kind="improvement_shift_within_design_rate")`, decided before any solve. It is the exact ARL of the two-sided joint chain (Decision 8 state space, Decision 13 integer lattices, calibration D's intervals) at the single true failure rate **`p̂ / M`**: the improvement the upper arm is tuned to detect. It mirrors `expected_detection_arl`'s `p̂ · M`. It is a number, not a warning: there is no power threshold (OQ-28). C13's advisory marks only the structural no-shift case. `None` for `"lower"`, whether requested or forced by f = 0, because no upper arm is checked. `None` for `"upper"`, because `expected_detection_arl` already *is* the improvement figure there, and carrying it twice would give two fields one meaning. **`p̂ = 0` never reaches it, so no fallback exists or is needed.** ~~It can exceed `achieved_arl`: at m=1000 f=5 it is 580.9 against `B` = 370.3~~. **Superseded by C13.** At m=1000 f=5 with M=2, `p̂/M` = 0.0025 ≥ `p_L` ≈ 0.00243. That is exactly the no-shift case, so the figure is now `None` with the advisory (`boundary = p̂/p_L` ≈ 2.05). Outside that condition, C13 bounds it above by the upper arm's own calibrated ARL (see "Improvement-detection power" below). C15.1 confirms it (`p̂/M` = 0.0025 ≥ `p_l` = 0.002435); that cell's degradation figure stays 558.0511 |
...
| `advisories` | `tuple[FittingAdvisory, ...]` | Defaults to `()`. **Four kinds exist for this chart** (Decisions 12 and 19.2, corrigendum C13). `FittingAdvisory` is the existing `kind`/`description`/`boundary: float` type (`fitting_advisory.py`); `description` is prose and not part of the contract. <br>• **`kind="lower_arm_signals_on_first_failure"`**, `boundary` = the exact one-sided lower-arm ARL₀ at `h_units = 1`, which equals `1/p_U`. It is present exactly when `direction` checks the lower arm (`"lower"` or `"two_sided"`) **and** `lattice_lower.decision_interval_units < lattice_lower.denominator − lattice_lower.reference_units`, which is an integer test. <br>• **`kind="upper_arm_not_designable"`**, `boundary=1.0`, the smallest baseline failure count at which the improvement arm can be designed. It is present exactly on a two-sided request against a zero-failure baseline (Decision 19.2). <br>• **`kind="detection_shift_within_design_rate"`** (C13), `boundary = p_U / p̂`: the `detect_rate_multiple` at which `p̂·M = p_U`. It is present exactly when `expected_detection_arl` is `None` for `"lower"`/`"two_sided"` (f ≥ 1 and `p̂·M ≤ p_U`). <br>• **`kind="improvement_shift_within_design_rate"`** (C13), `boundary = p̂ / p_L` (≥ 1, since `p̂ ≥ p_L`): the multiple at which `p̂/M = p_L`. It is present exactly when the improvement figure is `None` by C13's condition: `expected_detection_arl` for `"upper"`, or `expected_improvement_detection_arl` for `"two_sided"`. <br>⚠️ **Both C13 boundaries are exclusive**: only a *strictly* larger multiple gives a reported figure. That is the opposite of the F-row bounds, so a test must not assume inclusivity. They are disclosures only, and nothing refuses. <br>**Ordering:** the floor advisory first, then the C13 advisories, then `upper_arm_not_designable`. <br>Several can co-occur. An f = 0 two-sided request whose lower-only chart is floored (m=10,000 f=0 in 19.4's table) carries the floor advisory and `upper_arm_not_designable`. A two-sided fit meeting both C13 conditions carries both C13 advisories. Forward-compatible with ADR-005's unbuilt middle-tier adequacy advisory (ADR-013 §2) |
| `detect_rate_multiple` | `float` | *M*, the shift lever: a multiple of the in-control failure rate rather than a sigma multiple (ADR-012 §1, a structural consequence of a Bernoulli process having one parameter). It moves the lower design point *up* (`M·p_U`) and the upper design point *down* (`p_L/M`). Default `2.0` (ADR-012 §1, extended ADR-013 §6a). **Legal range** (corrigendum C2, option A ratified): `M` must be finite (F7) and greater than a **per-fit computed lower bound** that is always `> 1` (F16). `M ≤ 1` is refused: `M = 1` detects no shift, and `M < 1` would mislabel the arms. Constructibility is checked **directly at the requested `M`**, over **every arm the fit designs** (C12.1). A two-sided fit's bound is its own computation, not the larger of two per-arm bounds. It is **not monotone in `M`**: <br>• just above 1 the design outruns double precision (non-constructible multiples found only below `M − 1 ≈ 5.41×10⁻⁸` for m ≤ 300,000); <br>• on the **upper** arm, when `p_L ≲ 3.5×10⁻⁷` (very large, near-clean baselines), isolated or frequent multiples up to about 36 are also non-constructible (C14; e.g. m=3,000,000 f=1 at `M ≈ 4.1957`). The lower arm is unaffected. <br>F16's `min_value` is **the nearest constructible multiple at or above the request** (C12.1, searched upward per C14). It round-trips and moves the request by only a few float spacings. <br>**Ceiling `C`** (corrigendum C15.2): the smaller of F11's `(1 − 10⁻⁹)/p_U` when the lower arm is designed, and **`U = p_L · 2⁵³`** when the upper arm is designed. `U` is the largest multiple at which `1 − p_L/M` is representable strictly below 1. A request above `C` is refused by **F11, extended to the upper arm**, with `max_detect_rate_multiple = C` |
...
- So at `M = 2`:
  - the **degradation** figure is `None` for **f ≤ 3**, and the **improvement** figure is `None` for **f ≤ 5**, at every baseline size measured (m from 100 to 300,000);
  - for example, m = 100, 1,000 or 300,000 with f = 1, and m = 300 with f = 3, report neither figure.
- The chart itself is valid. Only the disclosure is absent, and its advisory gives the multiple above which it would be reported.
- The ADR's reasoning is that at f ≤ 3 the Clopper–Pearson bound exceeds twice `p̂`, so a doubling is still a rate the chart tolerates by design.
- Whether the default `M` should rise, or depend on `f`, is open to the product owner and is not modelled here. The feature file's mixed baselines (f = 10–11 at m ≈ 100) and its zero-failure baseline are unaffected. The power threshold is recorded as open by design, OQ-28.
...
- **Postcondition, raising** (Decision 13.4, extended by 19.6). Every ARL copied onto the artefact must satisfy all of:
  ...
  This covers `achieved_arl`, and `expected_detection_arl` and `expected_improvement_detection_arl` when they are computed. A violation raises `DegenerateBaselineError(reason="arl_not_computable")`, row F14. **Corrigendum C13 narrows what reaches it.** A disclosure figure inside its no-shift condition is never solved: it is `None` with an advisory, decided first. Outside that condition, each figure is bounded above by an ARL already computed (one-sided: `≤ achieved_arl`; two-sided: `≤` the relevant arm's calibrated ARL). So F14 on a disclosure figure would be a genuine solver defect, and it stays a raise rather than silently becoming `None`. **In normal operation F14's `figure` is only ever `"achieved_arl"`.** Before C13 it was reachable on an ordinary input: m=100 f=25 `"lower"` T=10⁵ M=1.01 refused a valid chart with F14 on `expected_detection_arl`. The sentinel may steer the calibration search but must **never** be copied into a field. No measured input reaches this path after Decisions 13.1–13.3, so it is a guard, not an expected outcome.
...
- **How `N` is found** (corrigendum C2 part 1, **corrected by C12.4**). Let `J = [r − ε·gap, r + ε·gap] ∩ (p₀, p₁)`, closed at the tolerance ends and open at the interval ends.
  - `N_any` is the denominator of the simplest rational in `J`, found by continued fractions (Stern–Brocot) in exact `Fraction` arithmetic. No smaller `N` has any `k/N` in `J`.
  - From `N_any`, walk forward applying Decision 7's ratified test exactly: `k = round(r·N)`, strictly inside the interval, within `ε·gap`.
  - The walk is bounded by `N_sym`, the simplest denominator in the symmetric interval of radius `min(ε·gap, r − p₀, p₁ − r)`, where an answer always exists in exact arithmetic.
  - If double precision cannot realise one, the design is **non-constructible** and F16 fires.
  - It matched the ratified linear rule on **6,994 of 6,994** design points (2,642 with `M > 3`, m up to 3,000,000). The forward walk took at most 1 step.
  - ⚠️ C2's first version (the unintersected interval) was wrong at large `M`: its 797-point check sampled only `M ≤ 3`.
  - Decision 7's rule itself is unchanged. What is withdrawn is the claim that `N ≤ ⌈1/(2ε·gap)⌉` always suffices, and the unchecked `return` of that bound. At large `M`, `r` drifts toward one end of the interval. PR #29's scan at `5b7e3a1` returned an off-tolerance lattice in 761 of 4,000 random lower-arm designs (C12.4, a new defect; every test used `M ≤ 5`).
...
**BIN-133 implementation status (re-checked 2026-09-24 against PR #29 at `3bd013d`, on `feat/BIN-133/bernoulli-cusum`, unmerged; read from the code, not inferred from ADR text).** Nothing below is on `trunk`. `3bd013d` ("implement ADR-014 amendment 2 and its corrigendum") superseded the `5b7e3a1` state this table previously recorded.

**Checked present at `3bd013d`** (read from `bernoulli_arm_lattice.py`, `fitted_bernoulli_cusum.py`, `fitted_artefact_base.py`, `bernoulli_cusum_fitting.py` and `monitor.py`):
- the `BernoulliArmLattice` module;
- `lattice_lower` and `lattice_upper`, both `BernoulliArmLattice | None`, with the four floats as `@computed_field`;
- `p_l`, and `expected_improvement_detection_arl: float | None`;
- no `limit_denominator` anywhere in `src/`;
- `"gicp_markov_chain_coupled_bound"`, and the coupled `B` with calibration D;
- the advisory kinds `lower_arm_signals_on_first_failure` and `upper_arm_not_designable`;
- the `reason` values `no_baseline_failures`, `arl_not_computable`, `no_shift_to_detect`, `shift_below_numerical_resolution` and `no_valid_multiple`;
- the upward, doubling `min_value` search, deliberately not C12.1's fixed anchor (C14);
- the finite-float backstop covering `float | None` (C7);
- the `Monitor` lattice guards emitting `provided_type`, and `Monitor` stepping `*_units` integers.

This is a name-and-structure check, not a behavioural re-verification of every row. That is `code-reviewer`'s job.

**Not built at `3bd013d`: corrigendum C13.** `expected_detection_arl` is still typed `float`, and neither `detection_shift_within_design_rate` nor `improvement_shift_within_design_rate` appears anywhere in `src/`. So the m=100 f=25 `"lower"` T=10⁵ M=1.01 input still refuses a valid chart with F14.

**Not built at `3bd013d`: corrigendum C15.2.** `_max_multiple` returns `math.inf` when the lower arm is not designed, so an upper-only `min_value` search has no ceiling. F11 is not extended to the upper arm.
...
*(New, ADR-014 Amendment 2 Decision 14.1, ratified 2026-09-24 (Q5). Built on unmerged PR #29 at `3bd013d` (`bernoulli_arm_lattice.py`).)*
...
| **`expected_improvement_detection_arl`** | On a two-sided Bernoulli CUSUM only, the exact ARL of the two-sided chart at the single true failure rate `p̂ / M`: the improvement its upper arm is tuned to detect, the mirror of `expected_detection_arl`'s `p̂ · M`. It is `None` whenever the chart does not check both arms: `"lower"` (including at `f = 0`), and `"upper"`, where `expected_detection_arl` already is the improvement figure. It is also `None` when `p̂/M ≥ p_L` (corrigendum C13), with `improvement_shift_within_design_rate`. Otherwise it is a number, not a warning, with no power threshold attached. It makes visible when the upper arm has essentially no power (`p̂·T ≪ 1`). | Baseline | ADR-014 Decision 19.6 |
...
| **Continued-fraction lattice finder** | How a Bernoulli CUSUM arm's lattice denominator `N` is found (ADR-014 corrigendum C2, corrected by C12.4). `N_any` is the simplest denominator of any rational in the tolerance window intersected with the open design interval, found by the Stern–Brocot construction in exact `Fraction` arithmetic. Then walk forward applying Decision 7's exact test, bounded by `N_sym`. It gives the same result as Decision 7's rule, in O(log N), and replaces the linear scan. That scan hung as `M → 1⁺` and, at large `M`, returned lattices outside the centring tolerance. | Baseline | ADR-014 corrigendum C2, C12.4 |
| **No-shift disclosure** (`detection_shift_within_design_rate`, `improvement_shift_within_design_rate`) | The `FittingAdvisory` kinds carried when a Bernoulli CUSUM disclosure figure is `None` because its target shift lies at or inside the chart's design rate (corrigendum C13). The figure would then describe no detection, and be no faster than the chart's own false-alarm spacing. Degradation: `boundary = p_U/p̂`. Improvement: `boundary = p̂/p_L`. Each boundary is the `detect_rate_multiple` at which the shift reaches the design rate, and is **exclusive**: only a strictly larger multiple gives a reported figure. Never a refusal. | Baseline | ADR-014 corrigendum C13 |
| **Constructible** (lattice design) | A Bernoulli CUSUM arm's design is constructible at a given `detect_rate_multiple` when double precision can realise a lattice satisfying Decision 7's invariants. It fails just above `M = 1`, and on the upper arm at ordinary `M` (up to about 36) when `p_L ≲ 3.5×10⁻⁷` (C14). Constructibility is not monotone in `M`, so F16 checks it at the requested `M`, and its `min_value` is the nearest constructible multiple at or above the request. Above the ceiling `C` (for the upper arm, `U = p_L · 2⁵³`, where `1 − p_L/M` rounds to 1) no design point exists, and F11 refuses (C15.2). | Baseline | ADR-014 corrigendum C2, C12.1, C14, C15.2 |
...
*(Ratified 2026-09-24. PR #29 at `3bd013d` emits the reason values of F14, F15 and F16. Its F14 row still fires for disclosure figures, because C13 is not built. Row-by-row key conformance is `code-reviewer`'s to confirm.)* **These rows are the whole contract.** Every row is an emission site, and every key listed is required. The ADR-002 rules that apply here:
...
**`provided` vs `provided_type`** (corrigendum C6, settling OQ-35, **scoped by C12.2**, settling OQ-38). The rule is the documented exception to ADR-002 §3 that BIN-143 already practises: a **type** failure reports `provided_type` and never the value, because a hostile object's `repr` can raise inside the error path, and a **value** failure on an exact-typed value reports `provided`. It applies **only** to:
- the `require_exact_str` family: F8's not-a-`str` path, and M3;
- the new `Monitor` lattice guards: M2 and its C4 extension.

**F1, F4 and F6 keep `provided` (the value)**, because they use the shared `require_type`/`require_real_number` guards, which every chart uses and which BIN-121's registry already audits. Moving those guards to `provided_type` library-wide is a recommended follow-up ticket, not done here. The registry asserts `provided_type` on the `require_exact_str` and lattice-guard rows, and `provided` on F1, F4 and F6.

**`fit_bernoulli_cusum`**

| # | Condition | Type | Required `context` |
|---|---|---|---|
| F1 | `baseline` is not a `Baseline` | `InvalidParameterError` | `parameter="baseline"`, `constraint`, `kind="invalid"`, `provided` (existing shared `require_type`; kept by C12.2) |
...
| F4 | `target_arl` is not a real number, or is a `bool` | `InvalidParameterError` | `parameter`, `constraint`, `kind="invalid"`, `provided` (existing shared `require_real_number`; kept by C12.2) |
...
| F6 | `detect_rate_multiple` is not a real number | `InvalidParameterError` | `parameter`, `constraint`, `kind="invalid"`, `provided` (shared `require_real_number`; kept by C12.2) |
...
| F11 | `M` above the ceiling `C`: `p_U × M ≥ 1` when the lower arm is designed; **extended by corrigendum C15.2** to `M > U = p_L · 2⁵³` when the upper arm is designed, where `1 − p_L/M` rounds to 1. `C` is the smaller applicable bound | `InvalidParameterError` | `parameter="detect_rate_multiple"`, `constraint` (now also naming `1 − p_L/M < 1`), `kind="invalid"`, `provided`, `max_detect_rate_multiple = C` (round-trips; the lower-arm bound is computed from `p_U`, not `p̂`, per ADR-013 §6b; the key name is ratified and unchanged) |
...
| F14 | **new:** a **computed** ARL fails its postcondition (Decision 13.4): non-finite, `< 1`, the sentinel, or a wrong state count | `DegenerateBaselineError` | `reason="arl_not_computable"`, `chart_type="bernoulli_cusum"`, and `figure`. **Narrowed by C13:** in normal operation `figure` is always `"achieved_arl"`. A disclosure figure inside its no-shift condition is `None` with an advisory and is never solved. Outside it, the figure is bounded by an ARL already computed. `"expected_detection_arl"` and `"expected_improvement_detection_arl"` stay in the enumeration (C10) only as a solver-defect guard, and the registry still enumerates all three |
...
| F16 | **new (corrigendum C2, option A ratified C-Q1; corrected by C12.1 and C14):** `detect_rate_multiple ≤ 1`, **or** the arms the fit designs are not constructible in double precision at the **requested** `M`, which is checked directly, not inferred from a threshold | `InvalidParameterError` | `parameter="detect_rate_multiple"`, `constraint`, `kind="invalid"`, `provided`, and `reason`: <br>• `"no_shift_to_detect"` for `M ≤ 1`; <br>• `"shift_below_numerical_resolution"` for `M > 1` not constructible; <br>• `"no_valid_multiple"` when no constructible multiple lies in `(request, C]`. This is a complete row (C15.2): `parameter`, `constraint`, `kind="invalid"`, `provided`, `reason`, and **no** `min_value` and no round-trip key (mirrors F10). The recovery hint directs the engineer to a smaller multiple. It was **not reached in any measured case** (C15.2, correcting C14's "measured unreachable", which had no recorded result), but it stays specified so every path terminates. <br>For the first two reasons, also `min_value` and `min_inclusive=True`. `min_value` is **the nearest constructible multiple at or above the request** (C12.1), over **every arm the fit designs**: `"lower"` → lower arm, `"upper"` → upper arm, `"two_sided"` → both at f ≥ 1 and the lower arm only at f = 0. It is found by the **upward search** of C14: start at the request (or 1 when `M ≤ 1`), step up by one float spacing, doubling until constructible, then bisect to adjacent floats and return the constructible one. It is **capped at the ceiling `C`** for every arm set (C15.2): F11's bound for the lower arm, `U = p_L · 2⁵³` for the upper. **It terminates**: the candidate strictly increases, reaching `C` within about 1,100 doublings, and the bisection takes about 1,100 halvings. That replaces production's `math.inf` ceiling for upper-only fits. It round-trips by construction, and measured moves from the request are ≤ 1.65×10⁻⁷ relative. It differs by direction on the same baseline. C12.1's fixed anchor at `M = 2` is withdrawn |
...
**Corrigendum C12–C14 (ratified 2026-09-24): OQ-37 is settled by C12.1 and C14, and OQ-38 by C12.2. C13 makes a disclosure figure `None` when its shift lies at or inside the design rate. Two new questions, 39 and 40, record where C13 and C14 conflict with earlier corrigendum text.**

**Corrigendum C15 (2026-09-24): OQ-39 is settled by C15.1, and OQ-40 by C15.2.** C15.3's default-`M` finding is recorded under `FittedBernoulliCUSUM` ("Improvement-detection power") as a noted consequence awaiting a product-owner ruling. It is not an open question of this model's.
...
| 37 | ~~Over which arms is F16's `min_value` computed? C2 defines it as the smallest `M` "for which both arms' design is constructible and verified". But under C3 a one-sided fit designs only its checked arm, and at f = 0 the upper arm is never designable. Read literally, a `"lower"` fit, or any f = 0 fit, either could never compute the bound, or would be bounded by an arm it does not build.~~ | — | — | Corrigendum C2 vs C3 | **SETTLED by corrigendum C12.1, refined by C14 (2026-09-24).** `min_value` is computed over **every arm the fit designs**: `"lower"` the lower arm, `"upper"` the upper arm, `"two_sided"` both at f ≥ 1 and the lower only at f = 0. It is the nearest constructible multiple **at or above the request**, found by C14's upward search. F16 itself is checked directly at the requested `M`. Measured: the bound differs by direction, and a two-sided bound is its own computation (m=100 f=60: 7.35e-9 against 3.53e-9 and 3.54e-9). |
| 38 | ~~Do F1, F4 and F6 report `provided` or `provided_type`? C6 says its rule "applies to every row in Decision 17": a type failure reports `provided_type`, never the value. But F1 (`baseline` not a `Baseline`), F4 (`target_arl` not a real number) and F6 (`detect_rate_multiple` not a real number) are type failures that Decision 17 lists with `provided`. On PR #29, `require_type` and `require_real_number` do emit `provided` (the value). C6 enumerates only F8, M2, C4 and M3.~~ | — | — | Corrigendum C6 vs Decision 17 F1/F4/F6 | **SETTLED by corrigendum C12.2.** C6's `provided_type` rule covers only the `require_exact_str` family (F8's non-`str` path, M3) and the new `Monitor` lattice guards (M2, C4). F1, F4 and F6 keep `provided` via the shared guards. Moving those library-wide is recommended as a separate ticket. |
| 39 | ~~C13 makes several of the ADR's own recorded disclosure figures `None`, and the ADR has not re-based them. Computed at the default M = 2 from the Clopper–Pearson bounds at α = 0.10: <br>• C1, item 3: m=300,000 f=1 `"two_sided"` T=370 expects `expected_detection_arl` 370.5428 and `expected_improvement_detection_arl` 370.8856. There `p̂·M` = 6.7e-6 ≤ `p_U` ≈ 1.30e-5, and `p̂/M` = 1.7e-6 ≥ `p_L` ≈ 3.5e-7, so both are `None` under C13. <br>• C1, item 8: `"upper"` m=1000 f=1 expects 398.1379; `p̂/M` = 5e-4 ≥ `p_L` ≈ 1.05e-4, so `None`. <br>• 19.6's measured table: m=100 f=1, m=1000 f=1 and m=300 f=3 are `None` on both figures, and the improvement figure at m=1000 f=5 (580.9, the ADR's own "above `B`" example) is `None`. <br>• 19.2's power discussion (398.1 vs 370.1) is now the C13 case.~~ | — | — | ADR-014 corrigendum C1 and 19.6 vs C13 | **SETTLED by corrigendum C15.1.** Every affected cell is re-based from the artefact's own `p_u`/`p_l`. The `None` cells stay in Decision 18 as assertions of `None` plus the matching advisory. They are: m=300,000 f=1 two-sided (both figures); m=1000 f=1 `"upper"`; 19.6's m=100 f=1, m=1000 f=1 and m=300 f=3 (both), and m=1000 f=5 (improvement only; detection stays 558.0511); and 19.2's `"upper"` rows. Replacement cells pin the non-`None` path: m=300,000 **f=30** two-sided (`B` 370.9495, detection 365.9542, improvement 376.4221) and `"upper"` (achieved 370.2214, detection 368.3603); `"upper"` 72.1128 / 157.6429; and 19.6's m=200 f=20, m=100 f=10 and m=150 f=40 two-sided figures. |
| 40 | ~~What bounds C14's upward `min_value` search for an `"upper"`-only fit, and is `no_valid_multiple` "measured unreachable"? C14 bounds the search by F11's `max_detect_rate_multiple` "when the lower arm is designed". An `"upper"` fit designs no lower arm, so no bound is specified: termination rests on the measurement that failures stop above M ≈ 36, not on a proof. C14 also says `no_valid_multiple` "was measured unreachable (C2, item 20)". But C2 only *assigns* that measurement to verification item 20, and records no result.~~ | — | — | ADR-014 corrigendum C14 vs C2 | **SETTLED by corrigendum C15.2.** The search is capped at a ceiling `C` for every arm set: F11's bound when the lower arm is designed, and `U = p_L · 2⁵³` when the upper arm is. F11 is extended to refuse `M > C` with `max_detect_rate_multiple = C`. The search provably terminates (about 1,100 doublings plus 1,100 halvings). `no_valid_multiple` is a complete row with no round-trip key. It was measured **not reached** in any case: 32 lower-designed extremes and seven upper-designed baselines on `[36, U]`. This corrects C14's uncited "measured unreachable". |
...
- [x] Corrigendum C12–C14 (ratified 2026-09-24) read in full and reflected (C13/C14 pass):
  - C12.1: `min_value` over the designed arms, at or above the request.
  - C12.2: `provided_type` scope; F1, F4 and F6 keep `provided`.
  - C12.4: exact finder correction and the new large-`M` defect.
  - C13: `expected_detection_arl: float | None`; `expected_improvement_detection_arl` narrowed; the two no-shift advisory kinds with exclusive boundaries and ordering; F14 narrowed to `achieved_arl` in normal operation.
  - C14: the upward `min_value` search; upper-arm unconstructibility at small `p_L`.
- [x] OQ-37 and OQ-38 marked SETTLED (C12.1/C14, C12.2). OQ-28 left open by design. OQ-39 (C13 vs C1/19.6's recorded figures) and OQ-40 (C14's unbounded upper-only search; "measured unreachable" cross-reference) recorded, not resolved by invention (C13/C14 pass)
- [x] Implementation status re-checked against PR #29 at `3bd013d` from the code: Amendment 2, C1–C12 and C14 present by name and structure; **C13 not built**. Stale "not built" markers corrected (C13/C14 pass)
- [x] Examples the model quoted that C13 makes `None` corrected rather than left standing: 580.9 at m=1000 f=5, and the 398.1 power example. The default-`M` consequence for small-`f` baselines was computed from the Clopper–Pearson bounds, and the feature file's baselines were checked to be unaffected (C13/C14 pass)
- [x] Corrigendum C15 reflected (C15 pass):
...
- 2026-09-24 (domain-modeller, BIN-133, following ADR-014 corrigendum **C12–C14**, ratified 2026-09-24; design-branch checkout `arch/BIN-133/adr-014-lattice-amendment` at `f8386a1`, working tree only):
  - **C13:**
    - `expected_detection_arl` becomes `float | None`: `None` exactly when its shift lies at or inside the design rate, decided before any solve. Decision 4's "unconditional" is struck through.
    - `expected_improvement_detection_arl` is also `None` when `p̂/M ≥ p_L`.
    - New advisory kinds `detection_shift_within_design_rate` (`boundary = p_U/p̂`) and `improvement_shift_within_design_rate` (`boundary = p̂/p_L`), both exclusive, with the ratified ordering.
    - The postcondition text and F14 are narrowed: `figure` is `"achieved_arl"` in normal operation, and the disclosure values remain only as a defect guard.
    - Quoted examples that C13 makes `None` (580.9, 398.1) are corrected, and the default-`M` consequence for small-`f` baselines is stated.
  - **C12 and C14:**
    - F16 is checked directly at the requested `M`, over the designed arms.
    - `min_value` is the nearest constructible multiple at or above the request, found by C14's upward doubling-then-bisect search; the fixed anchor is withdrawn.
    - Upper-arm non-constructibility at ordinary `M` when `p_L ≲ 3.5×10⁻⁷` is recorded.
    - The C6 scope is narrowed (F1, F4 and F6 keep `provided`).
    - The finder text is corrected to C12.4's exact finder, with the large-`M` defect.
  - **Implementation status:** re-checked against `3bd013d`; everything except C13 is present.
  - **Glossary:** two terms added (no-shift disclosure, constructible); three updated.
  - **Open Questions:** OQ-37 and OQ-38 SETTLED; new OQ-39 and OQ-40.
