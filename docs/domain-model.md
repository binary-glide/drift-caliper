...
**Stories:** BIN-63 (collection), BIN-64 (sufficiency), BIN-65 (EWMA fitting), BIN-66 (review), BIN-68 (provenance comparison), BIN-94 (CUSUM fitting), BIN-95 (Shewhart fitting), BIN-133 (Bernoulli CUSUM fitting for binary pass/fail rubrics — domain-modelled this pass, per ADR-012/013/014; not yet implemented).
...
| `record(observation)` | `ScoringResult` | `MonitoringResult` (now carrying `direction`, `fitted_artefact`, `delivery_failures` — ADR-010) | `ProvenanceMismatchError`, `InvalidObservationError` (incomplete `ScoringResult`; or, *domain-modelled BIN-133 following ADR-014 Decision 2, not yet implemented*, a non-binary score checked against a `FittedBernoulliCUSUM` — `context["reason"] == "score_not_binary"`). Never raises for a delivery failure — absorbed into the returned result (ADR-010) |
| `history` (property) | — | `tuple[MonitoringResult, ...]` | — |
| `clear_history()` | — | `None` | — |

🚨 **Constructor validation — SUPERSEDED 2026-09-12, corrected here 2026-09-14; widened here 2026-09-23 (domain-modelled, not yet implemented).**
...
**Since BIN-133 (domain-modelled this pass, ADR-014 §6a — not yet implemented), each concrete artefact has one further chart-specific shape decision worth naming here: `FittedBernoulliCUSUM` reports two decision intervals (`decision_interval_lower`, `decision_interval_upper`, one per arm), not one — a direct consequence of the two-sided-by-default design (ADR-012 amendment) combined with the shared-core protocol's continued exclusion of detection boundaries. This is additive to, not a revision of, the reasoning above.**

### `HasProvenance` — the narrower protocol a non-conforming artefact can still satisfy

*(ADR-004's 2026-09-12 amendment; documented here for the first time this pass, BIN-133, because `FittedBernoulliCUSUM` is the first concrete artefact that needs it — not yet implemented.)*
...
*(Domain-modelled this pass, BIN-133, following ADR-012/013/014 — not yet implemented; no `bernoulli`/`clopper_pearson`/`gicp` module exists in `src/` as of this writing.)*

🚨 **Does not satisfy `FittedControlLimits`** — the first fitted artefact type that does not (ADR-014 Decision 6a). Its statistic accumulates the raw pass/fail value directly against a probability-space reference value derived from a log-likelihood ratio over `(p_U, p₁)`; nowhere does a sigma estimate enter its calibration. It satisfies `HasProvenance` (above) and is otherwise its own type, reporting its own, honestly-scoped field set.

| Field | Type | Notes |
|-------|------|-------|
| `chart_type` | `str` | `"bernoulli_cusum"` |
| `observed_failure_rate` | `float` | p-hat = f / m. The one summary statistic ADR-012/013's own vocabulary uses throughout, and what Story 1's scenario names directly ("reports the observed failure rate") — no separate `baseline_mean`; reporting the complement too would be redundant |
| `observation_count` | `int` | m |
| `provenance_model_version` | `str` | `HasProvenance` |
| `provenance_criteria` | `str` | `HasProvenance` |
| `requested_arl` | `float` | The engineer's requested false-alarm ARL0 |
| `achieved_arl` | `float` | Exact, via the joint two-armed Markov-chain solve on independent per-arm lattices (ADR-014 amendment Decision 8) — see calibration and lattice invariant notes below |
| `expected_detection_arl` | `float` | ADR-013 §4's detection-performance disclosure. The exact ARL1 of the constructed chart, evaluated at `p̂ × detect_rate_multiple`, falling back to `p_U × detect_rate_multiple` when `f = 0` (§4's fallback: "a rise from zero" is undefined as a target rate otherwise). **First-class field, not routed through `FittingAdvisory`** (ADR-014 Decision 4) — every fit reports it unconditionally, on the main happy path (Story 1 and Story 2's scenarios both assert it), unlike `FittingAdvisory`'s genuinely-optional, conditional disclosures |
| `calibration_method` | `str` | `"gicp_markov_chain"` (the exact joint solve; preferred, ship-blocking if not ready) or, only if that solve proves intractable within the release timeline, `"gicp_markov_chain_harmonic_combination"` — named honestly either way, so the artefact never silently claims an exactness property it does not have (ADR-014 §6c) |
| `advisories` | `tuple[FittingAdvisory, ...]` | Defaults to `()`. Empty today — forward-compatible with ADR-005's still-unbuilt middle-tier baseline-adequacy advisory, which ADR-013 §2 confirms does apply to binary baselines unchanged |
| `detect_rate_multiple` | `float` | *M* — the shift lever, a multiple of the in-control failure rate rather than a sigma multiple (ADR-012 §1, a structural consequence of a Bernoulli process having one parameter). Default `2.0`, the measured, largest-defined-everywhere default (ADR-012 §1, extended ADR-013 §6a) |
| `alpha` | `float` | `0.10` (ADR-013 §3), fixed. Reported for auditability, **not settable** — exposing it would let an engineer choose a value outside the five-point verified grid with no guarantee behind it (ADR-014 Decision 5) |
| `p_u` | `float` | The one-sided Clopper–Pearson upper confidence bound on the baseline's observed failure rate — the conservative rate the design actually uses in place of `p̂` (Guaranteed In-Control Performance, ADR-013 §1) |
| `direction` | `str` | `"two_sided"` (default), `"lower"`, or `"upper"` — same vocabulary as `FittedCUSUM.direction`, same caveat as `MonitoringResult.direction` above (only outcomes, not this configuration field, are restricted to two of the three words) |
| `reference_value_lower` | `float` | *r* for the degradation-detecting (lower) arm — design point `p₁ = M · p_U` |
| `decision_interval_lower` | `float` | *h* for the degradation-detecting (lower) arm |
| `reference_value_upper` | `float` | *r* for the improvement-detecting (upper) arm — design point `p_U / M` (ADR-012 amendment §2, the same lever applied symmetrically) |
| `decision_interval_upper` | `float` | *h* for the improvement-detecting (upper) arm |

Both arms' `reference_value_*`/`decision_interval_*` fields are present **regardless of `direction`** — mirroring `FittedCUSUM.direction`'s "both sums always updated regardless of direction" convention (`Monitor._check_cusum`'s existing behaviour) — so an engineer inspecting the artefact can see what a direction change would produce without refitting.

No `target_value`/`sigma_estimate`/`sigma_estimation_method`/`baseline_spread` fields — deliberately absent (see `HasProvenance` above and the Dual-Spread narrowing note below); there is no raw-scale target or operational sigma for this chart to report.

**`p_U` substitutes for `p₀` in *both* arms' design-point formulas, consistently** (ADR-014 §6d — the literal, minimal application of ADR-013 §1's stated substitution rule to every place ADR-012's formula used the symbol it replaces). ⚠️ **The upper arm's own false-alarm coverage under this substitution is unverified — flagged, not resolved.** Heidema et al.'s GICP proof establishes its coverage guarantee for detecting a parameter *increase*, the direction where an upper confidence bound is the conservative choice. The upper arm here detects a *decrease* (an improvement event). Whether `p_U` is still the coverage-preserving choice for that direction, or whether it should instead use a *lower* confidence bound, has not been derived or measured. The `achieved_arl` **number** for the upper arm is still trustworthy for the parameters used (the exactness argument below still holds) — what is unverified is whether that design's *own* false-alarm probability meets the ratified `1 − α` standard. See Open Questions.

**Calibration is exact for both arms, via a joint two-armed Markov-chain solve on independent per-arm lattices — deliberately not `FittedCUSUM`'s harmonic-combination approximation** (ADR-014 §6c, amended by Decision 8). `cusum_fitting.py`'s `_combine_two_sided_arl0` (Montgomery Eq. 9.7) is itself an approximation, acceptable there because the continuous CUSUM's whole ARL0 computation (Siegmund's diffusion approximation) is already approximate. That reasoning does not transfer: ADR-012 §4/§5's entire justification for shipping the Bernoulli CUSUM *before* the Bernoulli EWMA is that its ARL0 is **exact** — a finite Markov chain, independently cross-checked. Reusing the approximate combination formula would silently reintroduce approximation into the one property that justified shipping this chart first. The decided method (ADR-014 amendment Decision 8): each arm's CUSUM accumulator lives on its **own** lattice with its own denominator `N` (computed by the per-arm adaptive rule, Decision 7 — see the lattice invariant below). State `(i, j)` means the lower arm is at `i` units of `1/N_lower` and the upper arm is at `j` units of `1/N_upper`. A single Bernoulli(*p*) draw transitions both: `X = 1` (failure, prob `p`) grows the lower arm and shrinks the upper; `X = 0` (success, prob `1 − p`) does the reverse. Absorption when either arm strictly exceeds its own decision interval. State count: `(h_lower_units + 1) × (h_upper_units + 1)`. Solved once at `p = p_U` (in-control) and once at the alternative hypothesis (`p̂ × M`, or the `f = 0` fallback). One-sided configurations are unaffected — ADR-012's existing single-arm exact solve already covers them fully; only the two-sided combination needs the joint solve. **The two arms do not need to share a lattice denominator** — the exact property (finite Markov chain, `(I − Q)m = 1`) is preserved because both arms share a single source of randomness, not a single lattice (ADR-014 amendment Decision 8). A shared LCM-based denominator was measured to inflate joint state counts by up to 218x at coprime per-arm Ns, making many configurations intractable — see the amendment for the full measurement table.

**Lattice quantisation invariant — per-arm adaptive denominator with centring tolerance** *(new, ADR-014 amendment Decision 7 — not yet implemented; the current implementation on `feat/BIN-133/bernoulli-cusum` uses a fixed `N = 100`, which violates the `(p₀, p₁)` interval invariant at low failure rates)*. Each arm computes its own lattice denominator `N`, independently, as the smallest integer such that the quantised reference value `r_q = round(r × N) / N` satisfies **both**: (a) `r_q` lies strictly inside `(p₀, p₁)` — the open interval ADR-012's derivation requires; and (b) `|r_q − r| ≤ ε × (p₁ − p₀)` where `ε = 0.25` — the centring tolerance, keeping `r_q` near `r` rather than at the edge of the interval (measured: without centring, `r_q` positions of 0.96–1.00 at the interval edge inflate joint state counts 2.5–3.5x at high targets). The implementation scans from `N = 2` upward rather than using a closed-form bound (which overshoots the minimum `N` by ~3.4x consistently, producing ~12x more joint states than necessary). The scan terminates because the closed form guarantees a safe `N` exists. The two arms need very different granularities (measured: at m=1000, f=0, lower arm needs N=257, upper needs N=514 at `ε = 0.25`), so a single denominator serving both would be `max(N_lower, N_upper)` or, worse, their LCM — the independent per-arm approach avoids this entirely.

🚨 **This invariant is the correctness fix the amendment exists for.** The amendment documents a defect found by post-approval code review: a fixed `N = 100` lattice denominator places `r_q` outside `(p₀, p₁)` whenever the gap `p₁ − p₀` is narrower than one lattice step — which it is for every `f = 0` baseline at `m ≥ 300` (the common case for a healthy agent on a binary rubric). The consequence ranges from destroyed detection (lower arm: both drifts negative, chart never signals) to unbounded calibration (upper arm: in-control drift positive, decision interval inflated without bound). All existing tests sat at `p₀ ∈ [0.02, 0.30]` and systematically excluded the defect region. See the amendment's §0 for the full defect table and root cause analysis.

**Resource bounds on `fit_bernoulli_cusum`** *(new, ADR-014 amendment Decisions 9, 10a, 10b — not yet implemented)*:

1. **`target_arl` ceiling at `MAX_MEANINGFUL_ARL` (= 1,000,000)** — Decision 9. `fit_bernoulli_cusum` rejects `target_arl > MAX_MEANINGFUL_ARL` with `InvalidParameterError` (`context["parameter"] = "target_arl"`, `context["kind"] = "invalid"`, `context["max_value"] = MAX_MEANINGFUL_ARL`, `context["max_inclusive"] = True`). The value `MAX_MEANINGFUL_ARL` round-trips as an accepted input (BIN-122's rule). The ceiling exists to bound cost (the per-arm decision interval search and joint state count), not to fix the lattice defect — confirmed by measurement: at `target_arl = 370` (well below 10⁶), the fixed `N = 100` lattice already violates the invariant. Checked before calibration.

2. **Search cap on per-arm calibration** — Decision 10a. When `_calibrate_one_sided_decision_interval_units`'s exponential search reaches `_MAX_DECISION_INTERVAL_UNITS` without finding a solution, it raises `InvalidParameterError` rather than returning silently. `context` reports the achievable ARL₀ at the cap (the ARL at `_MAX_DECISION_INTERVAL_UNITS`, the largest this arm can deliver — ARL is monotonically non-decreasing in *h*). That value round-trips as an accepted input, reported as `context["max_attainable_arl"]` (OQ-25, settled — mirrors the continuous CUSUM's `min_attainable_arl`). In practice, with Decision 7's centred lattice in place, this cap is a safety net rather than a common path — it fires only when the interval invariant is violated and the search compensates by inflating *h* without bound.

3. **Joint two-sided state count cap of 1,000,000** — Decision 10b, ratified by the product owner. When the joint state count `(h_lower_units + 1) × (h_upper_units + 1)` exceeds `_MAX_JOINT_STATES = 1,000,000`, the fit raises `InvalidParameterError` with `context["reason"] = "joint_state_count_exceeded"`, the computed state count, the cap, and `context["max_two_sided_target_arl"]` — the largest two-sided `target_arl` this baseline supports under the cap, computed via bisection (O(50) iterations of per-arm calibration, not a joint solve). That value **round-trips as an accepted input** (BIN-122's rule — verified at five baselines in the amendment). `recovery_hint` directs the engineer to: (a) reduce `target_arl` to at most `max_two_sided_target_arl`, (b) increase `detect_rate_multiple` (wider gap reduces per-arm decision intervals), or (c) use `direction = "lower"` or `"upper"`, which avoids the joint solve entirely and always succeeds up to `MAX_MEANINGFUL_ARL` (verified: one-sided state count at 10⁶ is at most ~40,000 at any tested baseline). Checked after per-arm calibration, before the joint solve. Memory is the binding constraint: at 1M states, peak RSS is ~908 MB (~1.3s) — under 1 GB, safe for a typical CI runner or agent container. At the ratified 1M cap, `target_arl = 370` fits **every** baseline; all baselines with `f ≥ 1%` of `m` fit at all targets up to 10⁶.

**Every reported bound must round-trip as an accepted input** — `MAX_MEANINGFUL_ARL`, the achievable ARL₀ at the search cap, and `max_two_sided_target_arl`. This is the same discipline BIN-122 established: a value the library reports as a bound must, when passed back in, be accepted. Stated as an invariant of `fit_bernoulli_cusum`'s design.

`FittedArtefactBase`'s "every float field finite" postcondition (BIN-142) still applies to this type: its `sigma_estimate` field-validator is declared `check_fields=False` and simply does not fire on a type that declares no field by that name (verified directly against `fitted_artefact_base.py` — no change needed there).

`__bool__` forbidden, `ConfigDict(frozen=True)`, `audit_summary()` — same conventions as every other `Fitted*` type (BIN-110, BIN-108).
...
| **Direction** (CUSUM) | Which direction(s) of drift are monitored. `"two_sided"` (default): both arms. `"lower"`: degradation only (score drifts down). `"upper"`: improvement only (baseline staleness). Two-sided default, configurable to one-sided (ADR-004 section 6). Same vocabulary and default on `FittedBernoulliCUSUM` (ADR-012 amendment; domain-modelled BIN-133). | Baseline | ADR-004 |
| **Bernoulli CUSUM** | The CUSUM control chart for a binary (pass/fail) rubric — a sequential likelihood-ratio test accumulating `max(0, · )` against a decision interval, exactly like the continuous CUSUM, but on a raw `0`/`1` statistic rather than an observation standardised by sigma. Ships ahead of the deferred Bernoulli EWMA because its ARL₀ is exact (a finite Markov chain), not approximate (ADR-012 §4/§5). Domain-modelled this pass (BIN-133, ADR-012/013/014); not yet implemented. | Baseline | ADR-012, ADR-013, ADR-014, BIN-133 |
| **`detect_rate_multiple`** (*M*) | The Bernoulli CUSUM's shift lever — the design point is `M` times the in-control failure rate, rather than a sigma multiple, because a Bernoulli process has one parameter (its variance, `p(1−p)`, is determined entirely by the rate). Default `2.0`, measured (not conventional) as the largest multiple defined across the whole legal domain (`p₀ < 0.5`). Replaces `reference_value` for this chart family. | Baseline | ADR-012 §1 |
| **Guaranteed In-Control Performance (GICP)** | The design principle the Bernoulli CUSUM is calibrated under: the chart is designed at an upper confidence bound on the baseline's observed failure rate (`p_U`), not the plain estimate (`p̂`), so that `P(true ARL₀ ≥ target) ≥ 1 − α` holds per-baseline, at the least favourable true rate consistent with the data — not merely on average across many baselines. From Heidema et al. (2026), *Biometrical Journal*, originally for Poisson counts; Bernoulli's exact analogue of the paper's Garwood construction is Clopper–Pearson. Supersedes the provisional uniform baseline floor (ADR-013 §2). | Baseline | ADR-013 |
| **`p_U`** / **Clopper–Pearson upper bound** | The one-sided Clopper–Pearson upper confidence bound on the baseline's observed failure rate — `p_U(f, m, α)`, the quantile function of `Beta(f+1, m−f)` at `1−α` (closed form `1 − α^(1/m)` when `f=0`). The conservative rate GICP substitutes for `p̂` in every place ADR-012's design rule references the in-control rate, in both arms of the two-sided design (ADR-013 §1, ADR-014 §6d). | Baseline | ADR-013 §1 |
...
| **`expected_detection_arl`** | The Bernoulli CUSUM's detection-performance disclosure — the exact `ARL₁` of the constructed chart, evaluated at `p̂ × detect_rate_multiple` (or `p_U × detect_rate_multiple` when `f = 0`, since "a rise from zero" is otherwise undefined as a target rate). Answers "how fast will this plausibly catch a real degradation", distinct from `achieved_arl`'s "is my false-alarm promise safe". A first-class, unconditional field on `FittedBernoulliCUSUM`, not routed through `FittingAdvisory` (ADR-014 Decision 4) — exists so a chart safe-but-practically-inert at a small, low-rate baseline is never left disclosing only its reassuring half (ADR-013 §4). Name proposed by ADR-013, not finally ratified until ADR-014 confirmed it. | Baseline | ADR-013 §4, ADR-014 Decision 4 |
...
|-----------|----------------|------------------------|-----------|
| `InvalidParameterError` | `invalid_parameter` | `parameter`, `constraint`, `kind` (`"missing"` or `"invalid"`) | Judge creation (BIN-57), criteria config (BIN-58), sufficiency threshold (BIN-64), fitting parameters (BIN-65/94/95), `Monitor` construction from a non-artefact or a structurally-conforming-but-unsupported artefact (BIN-69/BIN-120), direct construction of any `Fitted*` type with a non-finite or non-positive `sigma_estimate` (BIN-119, amended 2026-09-11); direct construction of any `Fitted*` type with **any** non-finite `float` field, not only `sigma_estimate` (BIN-142); a `direction` that is not a `str` at `fit_cusum` or at `Monitor.record()` (BIN-143); *domain-modelled BIN-133 following ADR-014 §1/§6b, not yet implemented:* `fit_bernoulli_cusum`'s `baseline` (`kind="invalid"`, `context["reason"] == "score_not_binary"`, plus the offending value and its position — raised when any recorded observation's score is not exactly `0.0`/`1.0`, checked before sufficiency, ADR-014 Decision 1 validation order step 2); `fit_bernoulli_cusum`'s `detect_rate_multiple` (`kind="invalid"`) when the implied design point is not a valid probability, `context["max_detect_rate_multiple"]` reporting the largest value that round-trips as an accepted input — **computed from `p_U`, not `p̂`** (ADR-013 §6b; the value accepted on resupply is bounded by `p_U`, so a bound reported from `p̂` would name a multiple GICP then refuses) — **except** when every baseline judgement failed, where the same `parameter`/`kind` carries `context["reason"] == "all_baseline_judgements_failed"` and **no** `max_detect_rate_multiple` key, because no multiple exists that would fix it (ADR-013 §5, folding the former all-failed-baseline `DegenerateBaselineError` case into this path); *domain-modelled BIN-133 following ADR-014 amendment Decisions 9/10a/10b, not yet implemented:* **three new resource-bound refusals on `fit_bernoulli_cusum`**: (1) `target_arl > MAX_MEANINGFUL_ARL` — `context["parameter"] = "target_arl"`, `context["kind"] = "invalid"`, `context["max_value"] = MAX_MEANINGFUL_ARL`, `context["max_inclusive"] = True`; the ceiling value round-trips as an accepted input (BIN-122); checked first, before calibration (Decision 9); (2) per-arm calibration search exceeding `_MAX_DECISION_INTERVAL_UNITS` — the search raises rather than returning silently; `context` reports the achievable ARL₀ at the cap, reported as `context["max_attainable_arl"]`, which round-trips (Decision 10a; key name OQ-25, settled); (3) joint two-sided state count exceeding `_MAX_JOINT_STATES = 1,000,000` — `context["reason"] = "joint_state_count_exceeded"`, plus the computed state count, the cap, and `context["max_two_sided_target_arl"]` (the largest two-sided `target_arl` this baseline supports under the cap, computed via bisection, round-trips as an accepted input); `recovery_hint` directs to reduce `target_arl`, increase `detect_rate_multiple`, or switch to `direction = "lower"`/`"upper"` which always succeeds (Decision 10b, cap ratified by the product owner 2026-09-23) |
...
| `InvalidObservationError` | `invalid_observation` | `reason`, `missing_fields` | Incomplete input to baseline (BIN-63), incomplete input to Phase II monitor recording (BIN-69, `Monitor.record()`, reused not a new category — ADR-009 §3); *domain-modelled BIN-133 following ADR-014 Decision 2, not yet implemented:* a Phase II observation checked against a `FittedBernoulliCUSUM` whose `.score` is not exactly `0.0` or `1.0` — `context["reason"] == "score_not_binary"`, `context["missing_fields"] == ()` (nothing is missing; the value present is simply outside the domain this chart can interpret — a docstring broadening, not a new category, per ADR-002 §3's existing conditional-presence precedent on `InvalidParameterError.provided`) |
| `InsufficientBaselineError` | `insufficient_baseline` | `have`, `need` | Fitting from too-small baseline (BIN-65/94/95); *domain-modelled BIN-133, not yet implemented:* `fit_bernoulli_cusum` below ADR-005's unchanged 100-observation floor (ADR-013 §2 — no new, higher, binary-specific floor ships) |
| `DegenerateBaselineError` | `degenerate_baseline` | `reason` (`"zero_variance"`, or, since BIN-119, `"non_finite_sigma_estimate"` / `"sigma_estimate_underflow"`, or, since BIN-142, `"non_representable_control_limits"`) | Fitting from zero-variance baseline (BIN-65/94/95); fitting from a baseline whose moving-range sigma estimate overflows to `inf` or underflows to exactly `0.0` (BIN-119, `_moving_range_sigma` in `spc_numerics.py`, shared by all three chart types). **`fit_bernoulli_cusum` never raises this category** (domain-modelled BIN-133, ADR-013 §5) — a zero-variance-shaped baseline for a binary chart is either `p̂=0` (designed normally at `p_U`) or `p̂=1` (refused as `InvalidParameterError`, the `p₁ ≥ 1` path above), and GICP's confidence-bound construction means neither needs the "statistically unusable despite meeting the count threshold" diagnosis this category exists for |
...
| Fit Bernoulli CUSUM | BIN-133 | `Baseline` (inferred binary at fit time — no declared "kind"), `target_arl`, optional `detect_rate_multiple`, optional `direction` | `FittedBernoulliCUSUM` | Baseline — **domain-modelled (ADR-012/013/014 + ADR-014 amendment 2026-09-23). Implementation on `feat/BIN-133/bernoulli-cusum` predates the amendment and requires lattice/resource-bound updates (Decisions 7–10).** Raises `InvalidParameterError` (`parameter="baseline"`, `reason="score_not_binary"` when any observation's score is not exactly `0.0`/`1.0`; `parameter="detect_rate_multiple"` when the design point is not a valid probability; `parameter="target_arl"` when exceeding `MAX_MEANINGFUL_ARL` ceiling — Decision 9; per-arm calibration search cap exceeded — Decision 10a; joint two-sided state count exceeding 1,000,000 cap, reporting `max_two_sided_target_arl` — Decision 10b — see Error Contract Reference), `InsufficientBaselineError` (ADR-005's unchanged 100-floor, ADR-013 §2). **Never raises `DegenerateBaselineError`** — unlike the three continuous charts, neither `p̂=0` (designed at `p_U`, ADR-013 §5) nor `p̂=1` (falls into the existing `p₁ ≥ 1` `InvalidParameterError` path, ADR-013 §5) needs a bespoke degenerate-baseline branch |
