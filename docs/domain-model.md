...
| `achieved_arl` | `float` | Exact, via the joint two-armed Markov-chain solve — see calibration note below. **Two-sided calibration is unimplemented numerical work — see Open Questions** |
...
**Calibration is exact for both arms, via a joint two-armed Markov-chain solve — deliberately not `FittedCUSUM`'s harmonic-combination approximation** (ADR-014 §6c). `cusum_fitting.py`'s `_combine_two_sided_arl0` (Montgomery Eq. 9.7) is itself an approximation, acceptable there because the continuous CUSUM's whole ARL0 computation (Siegmund's diffusion approximation) is already approximate. That reasoning does not transfer: ADR-012 §4/§5's entire justification for shipping the Bernoulli CUSUM *before* the Bernoulli EWMA is that its ARL0 is **exact** — a finite Markov chain, independently cross-checked. Reusing the approximate combination formula would silently reintroduce approximation into the one property that justified shipping this chart first. The decided method: state space the Cartesian product of both arms' bounded lattices (each individually finite per ADR-012 §4's argument), a single Bernoulli(*p*) draw updating both accumulators each step, solved once at `p = p_U` (in-control) and once at the alternative hypothesis (`p̂ × M`, or the `f = 0` fallback). One-sided configurations are unaffected — ADR-012's existing single-arm exact solve already covers them fully; only the two-sided combination needs new work. **This is real, new, unverified numerical work that no ADR supplies** — see Open Questions.
...
| Fit Bernoulli CUSUM | BIN-133 | `Baseline` (inferred binary at fit time — no declared "kind"), `target_arl`, optional `detect_rate_multiple`, optional `direction` | `FittedBernoulliCUSUM` | Baseline — **domain-modelled this pass, ready for backend-test-writer (ADR-012/013/014). Not yet implemented** — no `bernoulli`/`clopper_pearson`/`gicp` module exists in `src/`. Raises `InvalidParameterError` (`parameter="baseline"`, `reason="score_not_binary"` when any observation's score is not exactly `0.0`/`1.0`; `parameter="detect_rate_multiple"` when the design point is not a valid probability — see Error Contract Reference), `InsufficientBaselineError` (ADR-005's unchanged 100-floor, ADR-013 §2). **Never raises `DegenerateBaselineError`** — unlike the three continuous charts, neither `p̂=0` (designed at `p_U`, ADR-013 §5) nor `p̂=1` (falls into the existing `p₁ ≥ 1` `InvalidParameterError` path, ADR-013 §5) needs a bespoke degenerate-baseline branch |
...
- `target_arl`: **optional in Python signature** (default `None`), **required by Caliper validation**. Omitting raises `InvalidParameterError(kind="missing")`. **Same optional-with-required-semantics shape for `fit_bernoulli_cusum`** (BIN-133) — but see the Open Questions note below: ADR-011's hard-floor/advisory-band *numbers* do not transfer to this chart, even though the missing-value mechanism does.
...
**Five more, added this pass (domain-modeller, BIN-133, following
...
drift between the two documents. Two (23-24) are genuinely new, introduced
— not closed — by ADR-014 itself: the upper arm's own false-alarm coverage
under the GICP substitution (Decision 6d), and the exact joint two-armed
Markov-chain solve the two-sided artefact's `achieved_arl`/
`expected_detection_arl` depend on (§6c), which is real, unverified
numerical work no ADR supplies. Neither is this pass's to close — modelled
as open, not as settled behaviour, per the domain-modeller's remit not to
invent numbers or resolve statistical questions no ADR has decided.
...
| 24 | **What is the exact joint two-armed Markov-chain construction for the two-sided `FittedBernoulliCUSUM`'s `achieved_arl`/`expected_detection_arl`?** | Whoever implements `fit_bernoulli_cusum`'s two-sided path, to ADR-013 §7's verification bar | `achieved_arl`, `expected_detection_arl`, and `calibration_method` for every `direction="two_sided"` fit — the release's own default | ADR-014 §6c (new, introduced by this ADR — the *method* is decided, the derivation is not) | **Open — explicitly not this pass's to close.** ADR-014 §6c decides *which method must be used* (state space the Cartesian product of both arms' bounded lattices, solved once at `p = p_U` and once at the alternative hypothesis) and *which method must not be reused* (`FittedCUSUM`'s harmonic-combination approximation — it would silently reintroduce approximation into the one property, exactness, that justified shipping this chart before the Bernoulli EWMA). It does not supply the derivation itself — real, new, unverified numerical work, to the same rigour ADR-012/013 already demonstrated (a postcondition on every solve, at least two independent implementations, judged on a confidence bound where applicable, posted to the ticket in full — ADR-013 §7). A named, honest fallback exists if the exact solve proves intractable within the release timeline: `calibration_method = "gicp_markov_chain_harmonic_combination"` rather than `"gicp_markov_chain"`, so the artefact never silently claims an exactness property it does not have — but the exact solve is preferred and ship-blocking if not ready. One-sided configurations (`direction="lower"`/`"upper"`) are unaffected; ADR-012's existing single-arm exact solve already covers them. |
...
- [x] Two things ADR-014 records as open, not decided, modelled as Open Questions (#23, #24) rather than as settled behaviour: the upper arm's own false-alarm coverage under the `p_U` substitution, and the exact joint two-armed Markov-chain solve for the two-sided artefact — neither invented, both flagged (this pass)
